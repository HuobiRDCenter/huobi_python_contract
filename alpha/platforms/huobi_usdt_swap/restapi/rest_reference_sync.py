...
class RestReferenceUsdtSwap:
    def __init__(self, host=None):
        if host is None:
            host = "api.hbdm.com"
        self.host = host

    def swap_unified_account_type(self, params: dict = None) -> json:
        path = "/linear-swap-api/v3/swap_unified_account_type"
        return get(self.host, path, params)

    def swap_switch_account_type(self, params: dict = None) -> json:
    ...
    def summary(self, params: dict = None) -> json:
        path = "https://status-linear-swap.huobigroup.com/api/v2/summary.json"
        return get(self.host, path, params)
